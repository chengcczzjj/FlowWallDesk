...
| 文件 | 用途 | 更新时机 |
|------|------|---------|
| `doc/dev-log.md` | 开发日志：每次变更的详细记录（做了什么、改了哪些文件、遇到什么问题） | 每次记录进度 / 提交时 |
| `doc/project-status.md` | 项目进展全景：模块完成度、功能清单、架构概览、待办规划 | 有里程碑式进展时更新（新模块完成、架构变更等） |
...
**Git Commit**: `<commit hash>` — <commit message>（提交后回填）
...
1. **读取现有日志**: 读取 `doc/dev-log.md`，了解已有记录，避免重复
...
7. **更新日志**: 回填最近一条（`Git Commit: （未提交）`）的 commit hash 和 message
...
3. 不提交 `node_modules/`、`dist/`、`out/` 等（已在 .gitignore）
4. commit message 简述用英文，body 可中英混合
5. 大量文件变更时考虑拆分提交

## 项目进展更新规则（project-status.md）

以下情况需要同步更新 `doc/project-status.md`：
- 某个组件/模块从"未实现"变为"已实现"
- 新增了组件类型或模块
- 架构发生变更
- 解决了阻塞性问题导致整体进展推进
...
- 如果 `doc/dev-log.md` 不存在，创建它并写入 header `# 灵月桌面 开发日志`
- 如果 `doc/project-status.md` 不存在，参照现有模板创建
