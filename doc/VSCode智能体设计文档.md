...
> 历史参考（2026-09-05 标注）：本文保留 2026-05-17 的工作区 Agent 设计快照，仅供高级文件/命令能力开发参考。本文中的“当前”指原记录时点；伴侣优先方向与真实实现状态见 [项目状态](../TempFile/文档资料/project-status.md)。它不是 Codex/Copilot 的执行规则，规划中的路由、工具与审批 UI 不能直接视为已实现。

> 最后更新：2026-05-17
> 读者：产品设计、Agent 架构、提示词工程、前后端实现和后续接手开发者
> 本文用于整理 VS Code / Copilot Agent 类工作区智能体的整体设计，并映射到灵月桌面现有本地工作区 Agent。它聚焦“像 VS Code 一样在项目里工作的 Agent”；原文曾引用的 `AI对话与智能体设计说明.md` 当前不存在，请按 [知识索引](../TempFile/文档资料/knowledge-index.md) 选择已有资料。
