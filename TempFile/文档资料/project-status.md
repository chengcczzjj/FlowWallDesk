...
| 自动更新         | ✅ 完成 | 启动检查、侧栏按需下载、下载进度、低优先级安装、按钮重启安装与状态推送 |
| 开机启动         | ✅ 完成 | 正式安装版默认注册 Windows 启动项，设置页可开关            |
| 正式分发         | ✅ v1.0.6 | NSIS 安装包、`latest.yml`、blockmap 与自动更新链路验证完成 |
...
| 图标收纳/Dock | ✅ 完成 | 桌面图标拖入托管、启动、删除恢复、跨壁纸全局保存；全屏恢复时重建交互状态，原生点击兜底仅在 Canvas 为光标处最上层窗口时启用，支持唤醒启动器子目录和 helper 进程的真实主窗口 |
...
| 组件生成与组件控制   | ✅ 90%  | 安全声明式生成协议、工具调用、桌面落地、配置持久化和管理页均已完成 |
...
- [ ] 图标收纳单项移回桌面与拖出恢复体验细化
- [ ] 将更新产物发布到公开 GitHub Release/公共对象存储，使终端用户可访问更新源
