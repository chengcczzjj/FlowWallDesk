...
| 自动更新         | ✅ 完成 | 启动检查、后台下载、退出安装、按钮重启安装与状态推送       |
| 开机启动         | ✅ 完成 | 正式安装版默认注册 Windows 启动项，设置页可开关            |
| 正式分发         | ✅ v1.0.2 | NSIS 安装包、`latest.yml`、blockmap 与自动更新链路验证完成 |
...
| AI 生成组件    | ✅ 完成 | 对话直接生成清单/倒计时/指标等声明式组件，支持交互、管理和移除 |
...
| 流式对话基础         | ✅ 完成 | AI SDK v6 流式输出、DeepSeek 专用工具循环、事件持久化到 SQLite |
