...
> 源码与隔离验证：2026-09-05；最近本机安装验证记录：2026-09-04。
> 源码版本仍为 1.1.11；09-05 的修复尚未打包或安装，不能等同于本机现有安装版。按已有记录，1.1.11 尚未上传 GitHub Release；本轮未执行安装或发布。
...
新增 `npm.cmd run test:electron:smoke`：先生产构建，再用隐藏 Electron 窗口、独立临时 userData 与合成资源验证协议沙箱和生产壁纸渲染器，不加载应用正常入口或真实桌面数据。这是有限的集成冒烟测试，不是完整 E2E。

最近设备验证仍限于 09-04 本机单屏安装态；模拟多屏、合成视频时钟测试与 Win32 直接调用不能替代目标公司环境验收。
