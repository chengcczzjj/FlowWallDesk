...
> 最后更新：2026-08-04
> 版本：1.0.4（智能组件与 DeepSeek V4 适配版）
...
| 自动更新         | ✅ 完成 | 启动检查、侧栏按需下载、下载进度、按钮重启安装与状态推送   |
| 开机启动         | ✅ 完成 | 正式安装版默认注册 Windows 启动项，设置页可开关            |
| 正式分发         | ✅ v1.0.4 | NSIS 安装包、`latest.yml`、blockmap 与自动更新链路验证完成 |
...
| 图标收纳/Dock | ✅ 完成 | 桌面图标拖入托管、启动、删除恢复、跨壁纸全局保存；全屏恢复时重建交互状态并以原生左键兜底漏失的 `pointerdown`，支持唤醒启动器子目录和 helper 进程的真实主窗口 |
