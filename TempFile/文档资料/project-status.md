...
> 1.1.12 已于 09-06 发布到 GitHub Release。1.1.13 在其上合并聊天伙伴组件控制、自动更新可靠性与桌面组件层修复，改由 GitHub Actions（windows-latest）构建、核验并发布；远端结果以 Release 复核为准，尚未在 Windows 实机安装验收。
...
| 发行 | [1.1.12](../../doc/发布说明/1.1.12.md) 已远端发布；[1.1.13](../../doc/发布说明/1.1.13.md) 起由 [Windows 发布流程](../../.github/workflows/release-windows.yml) 在 CI 构建、核验清单/哈希/asar 后先草稿再发布，同版本已发布即拒绝覆盖 | 推送 tag 触发；手动运行需工作流已在 main（发布时在构建提交上建 tag）；未配置 Windows 代码签名证书 |
...
- [ ] 1.1.13 Windows 实机安装验收：开应用/切输入法时组件不闪烁、指针不跳变，便利贴一次即可输入/拖动；自动更新从 1.1.12 升级。配置 Windows 代码签名。
