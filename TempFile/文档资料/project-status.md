...
| 发行 | [1.1.12](../../doc/发布说明/1.1.12.md) 已远端发布；[1.1.13](../../doc/发布说明/1.1.13.md) 起由 [Windows 发布流程](../../.github/workflows/release-windows.yml) 在 CI 构建、核验清单/哈希/asar 后先草稿再发布，同版本已发布即拒绝覆盖 | 推送 tag 或在 Actions 手动运行（发布时在构建提交上建 tag）；未配置 Windows 代码签名证书 |
