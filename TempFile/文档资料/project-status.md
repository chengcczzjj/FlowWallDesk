...
| 正式分发         | ✅ v1.0.8 | NSIS 安装包、`latest.yml`、blockmap 与自动更新链路验证完成 |
...
| 图标收纳/Dock | ✅ 完成 | 桌面图标拖入托管、启动、删除恢复、跨壁纸全局保存；原生点击兜底覆盖 Dock 与全部收纳并校验 Canvas 前台归属，支持真实主窗口唤醒、macOS 节奏的等高匀速等待弹跳和无动画 Win+D 回到桌面 |
