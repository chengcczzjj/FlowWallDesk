...
| 正式分发         | ✅ v1.0.9 | NSIS 安装包、`latest.yml`、blockmap 与自动更新链路验证完成 |
...
| 图标收纳/Dock | ✅ 完成 | 桌面图标拖入托管、启动、删除恢复、跨壁纸全局保存；原生点击兜底覆盖 Dock 与全部收纳并校验 Canvas 前台归属，Dock 与收纳图标统一为缩放回弹和图标扩散反馈，Win+D 回到桌面保持无动画 |
