...
| 配置绑定壁纸 | ✅ 完成 | 内置默认配置随 `assets/wallpaper/<id>/widget-config.json` 读取；用户拖拽、删除、AI 调整后的覆盖配置写入 userData |
| 切壁纸联动   | ✅ 完成 | 自动加载/清空组件                                |
...
| 双显示器设置   | ✅ 完成 | 主屏/复制/按屏设置/跨屏延展四种模式由统一持久化状态驱动；普通多屏使用每台显示器独立原生壁纸窗口，延展使用虚拟桌面窗口，支持可视化拓扑、热插拔和壁纸库直接切换布局/指定屏幕 |
...
| 开应用/切输入法时组件闪烁、需多次点击 | Electron `setFocusable()` 会重新登记任务栏并 `Deactivate()`；便利贴获焦会把画布抬到所有应用之上；主进程按存储矩形、renderer 按 DOM 各自判断命中 | 仅在状态变化时切换可聚焦并回到桌面层；命中以 renderer 上报的 DOM 区域为准，被其他窗口遮住时保持穿透 |
| 多屏毛玻璃错位                     | 只抽主屏壁纸帧并拉伸到虚拟桌面，偏移叠加 `screenX` | 每个壁纸窗口独立抽帧，按画布坐标逐块对齐 |
| 多屏 Dock/新组件跨越屏幕缝或落在空白区 | 落位和边界约束使用所有显示器的并集矩形 | 默认落在主显示器工作区，拖放按所在显示器约束，不可见组件移回最近显示器 |
