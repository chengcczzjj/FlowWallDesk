...
| 正式分发         | ✅ v1.1.7 | 启动锁屏后 Canvas 输入恢复版已生成 Windows NSIS 安装包、完成本机覆盖安装，并发布 GitHub Release 及自动更新资产 |

### 组件系统 ✅ 99%

| 功能         | 状态    | 说明                                             |
| ------------ | ------- | ------------------------------------------------ |
| 组件增删改   | ✅ 完成 | 编辑模式拖拽与非编辑模式安全长按移动，支持网格吸附、边界约束、重叠避让和吸附预览 |
...
| 组件 | 状态 | 说明 |
| --- | --- | --- |
| 自由便利贴 | ✅ 完成 | 一张任务一张纸；无需编辑模式即可输入、像素级拖动与折角缩放，支持重叠/多实例、字体/字号/粗斜下划线/删除线/列表/图片排版、桌面纸色/分类快捷设置、短撕除动画和全屏返回后的层级/命中自愈；完整管理位于主应用工作台，AI 跨实例管理 |
