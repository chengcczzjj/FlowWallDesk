...
> 历史技术方案：第一版骨架已经存在，本文不再作为当前任务清单；产品原则允许根据需求修订，不是不可推翻的命令。资料分工见 [知识索引](../knowledge-index.md)。

更新时间: 2026-04-27
读者: 后续负责落地代码的 AI / 开发者
基于: `memory-system-design.md`（当时的产品设计输入；后续修订以当前产品方向为准）
配套: `memory-system-technical-architecture-GPT-5.md`（远期技术细节，本文不重复）

> 本文只覆盖**第一版要做的事**：建骨架 + 跑通最小聊天闭环。UI 设计、Phase 2+ 能力都不在本文范围。
>
> 2026-05-25 方向修正：本文是早期落地参考，当前项目已具备聊天和工具骨架。最新产品主线是“AI 伴侣对话 / 轻量桌面助手”，复杂本地文件 Agent 能力只作为高级辅助储备，不再作为默认体验目标。
