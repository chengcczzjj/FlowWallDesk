...
## [2026-09-06] 发布 LingyueDesk 1.1.12 更新资产

**变更摘要**: 将已完成本机验收的 1.1.12 正式发布到 GitHub Release，使公司电脑可通过源码分支和 electron-updater 更新源取得同一版本。

**涉及模块**:
- `origin/codex/dock-foreground-input-guard`: 推送包含 UI 修复、发布规则和文档记录的源码提交。
- `v1.1.12` Release：上传 NSIS 安装包、对应 `.blockmap` 与 `latest.yml`；不上传 `win-unpacked`、备份、日志或缓存。
- `project-status.md` / `doc/发布说明/1.1.12.md`: 更新远端发布状态与本次构建的真实哈希/大小。

**验证结果**:
- `npm.cmd test`：类型检查、只读 lint 与全部 105 项测试通过（构建产物来自已验收的 1.1.12 本地构建）。
- 源码分支已推送，远端 SHA 与本地 `da44567` 一致；`v1.1.12` 为正式 Release，三类更新资产均已上传。
- 远端 Release 资产大小、`latest.yml` 中安装包大小/SHA-512、下载地址及更新源可发现性已核验；本机已安装并运行 1.1.12。
