...
import {
  Clock as ClockIcon, Calendar, Monitor, Plus, Type, Minus,
  CloudSun, TrendingUp, Newspaper, Wrench, Cat, Music, Activity,
  ChevronLeft, ChevronRight, X, CloudRain, Sun, Disc, Settings, RefreshCw,
} from 'lucide-react'
...
export function WidgetsPage({ subPage }: { subPage: string }) {
  ...
  const [stocksPreview, setStocksPreview] = useState<{ code: string; name: string; price: number; changePercent: number; change: number }[]>([])
  ...
  const addToDesktop = async (c: WidgetCatalogItem, config?: Record<string, unknown>) => {
    ...
    const { w, h } = c.floating && FLOATING_DESKTOP_SIZES[c.type]
      ? FLOATING_DESKTOP_SIZES[c.type]
      : WIDGET_SIZES[c.size]
  ...
  const removeFromDesktop = async (type: string) => {
    ...
    if (inst) { await window.lingyue.widget.remove(inst.id); refresh() }
  }
  ...
  // 初始加载预览数据
  useEffect(() => { refreshNewsPreview() }, [])
  useEffect(() => { refreshStocksPreview() }, [])
        ...
        {subPage === 'widgets-card' && (
          ...
          <div className={`comp-card ${WIDGET_SIZES.large.css} comp-card--bordered`}>
                ...
                <div style={{
                  ...
                  {stocksPreview.slice(0, 6).map((stock) => (
                    <div key={stock.code} style={{
                      ...
                      <div style={{ fontSize: 11, color: stock.changePercent > 0 ? '#c42b1c' : stock.changePercent < 0 ? '#0f7b0f' : '#666' }}>
            ...
            <CardAction
              ...
              onSettings={() => { loadConfigFromInstance('stocks'); setSettingsDialog('stocks') }}
          ...
          <div className={`comp-card ${WIDGET_SIZES['medium-v'].css} comp-card--bordered`}>
                ...
                <div style={{
                  ...
                  {newsLoading && newsPreview.length === 0 ? (
                    ...
                    <>
                      ...
                      }}>{newsPreview[0].title}</div>
                      {(newsPreview[0] as any).hot && (
                        <div style={{ fontSize: 9, opacity: 0.7, marginTop: 3 }}>🔥 {(newsPreview[0] as any).hot}</div>
                      )}
                    </>
                  ) : (
            ...
            <CardAction
              ...
              hasSettings
              onAdd={() => addToDesktop(CARD_WIDGETS[1], { source: newsSource, maxItems: newsMaxItems, refreshInterval: newsRefresh })}
              onRemove={() => removeFromDesktop('news')}
              onSettings={() => { loadConfigFromInstance('news'); setSettingsDialog('news') }}
          ...
          <div className={`comp-card ${WIDGET_SIZES.small.css} comp-card--bordered`}>
            ...
            <CardAction added={addedTypes.has('calendar')} onAdd={() => addToDesktop(CARD_WIDGETS[2])} onRemove={() => removeFromDesktop('calendar')} />
          </div>

          {/* 快捷工具 medium 2×1 */}
          <div className={`comp-card ${WIDGET_SIZES.medium.css} comp-card--bordered`}>
            <div className="comp-content" style={{ padding: 20, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }}>
              <ToolIcon label="便签" />
              <ToolIcon label="截图" />
              <ToolIcon label="设置" />
              <ToolIcon label="重启" />
            </div>
            <CardAction added={addedTypes.has('quicktools')} onAdd={() => addToDesktop(CARD_WIDGETS[3])} onRemove={() => removeFromDesktop('quicktools')} />
          </div>

          {/* 桌面萌宠 small 1×1 */}
          <div className={`comp-card ${WIDGET_SIZES.small.css}`} style={{ background: '#FDF9F3', border: '1px solid var(--border-card)' }}>
            ...
            <CardAction added={addedTypes.has('pet')} onAdd={() => addToDesktop(CARD_WIDGETS[4])} onRemove={() => removeFromDesktop('pet')} />
          </div>
          ...
          <div className={`comp-card ${WIDGET_SIZES.medium.css} comp-card--bordered`}>
            ...
            <CardAction added={addedTypes.has('sysmonitor')} onAdd={() => addToDesktop(CARD_WIDGETS[5])} onRemove={() => removeFromDesktop('sysmonitor')} />
          </div>
        </div>
          </>
        )}
...
function CardAction({
  added, hasSettings,
  onAdd, onRemove, onSettings, onRefresh,
}: {
  added: boolean; onAdd: () => void; onRemove: () => void
  hasSettings?: boolean; onSettings?: () => void; onRefresh?: () => void
}) {
  return (
    <>
      {added ? (
...
function WidgetSettingsDialog({
  type,
  newsSource, newsMaxItems, newsRefresh,
  stockSymbols, stockRefresh,
  onNewsSourceChange, onNewsMaxItemsChange, onNewsRefreshChange,
  onStockSymbolsChange, onStockRefreshChange,
  onConfirm, onClose,
}: {
  type: 'news' | 'stocks'
  newsSource: string; newsMaxItems: number; newsRefresh: number
  stockSymbols: StockSymbol[]; stockRefresh: number
  onNewsSourceChange: (v: string) => void; onNewsMaxItemsChange: (v: number) => void; onNewsRefreshChange: (v: number) => void
  onStockSymbolsChange: (v: StockSymbol[]) => void; onStockRefreshChange: (v: number) => void
  onConfirm: () => void; onClose: () => void
}) {
  const labelStyle: React.CSSProperties = { fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }
  const selectStyle: React.CSSProperties = {
    width: '100%', padding: '8px 10px', borderRadius: 6,
    border: '1px solid var(--border-control)', background: '#fff',
  ...
  return (
    ...
    >
      <div style={{
        width: 420, maxHeight: '80vh', borderRadius: 12,
        background: 'var(--bg-solid)', boxShadow: 'var(--shadow-flyout)',
        display: 'flex', flexDirection: 'column', overflow: 'hidden',
      }}>
        {/* 头部 */}
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '16px 20px', borderBottom: '1px solid var(--border-divider)',
        }}>
          <span style={{ fontSize: 15, fontWeight: 600 }}>
            {type === 'news' ? '新闻设置' : '自选股设置'}
          </span>
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
          {type === 'news' ? (
            <>
              <div>
                ...
                <select value={newsSource} onChange={(e) => onNewsSourceChange(e.target.value)} style={selectStyle}>
                  {NEWS_SOURCE_OPTIONS.map((o) => <option key={o.id} value={o.id}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <div style={labelStyle}>显示条数</div>
                <select value={newsMaxItems} onChange={(e) => onNewsMaxItemsChange(Number(e.target.value))} style={selectStyle}>
                  {[3, 5, 8, 10].map((n) => <option key={n} value={n}>{n} 条</option>)}
                </select>
              </div>
              <div>
                <div style={labelStyle}>自动刷新间隔</div>
                <select value={newsRefresh} onChange={(e) => onNewsRefreshChange(Number(e.target.value))} style={selectStyle}>
                  {[5, 10, 30, 60].map((n) => <option key={n} value={n}>{n} 分钟</option>)}
                </select>
              </div>
            ...
            <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {stockSymbols.map((s) => (
                    <div key={s.code} style={{
                      ...
                      <span>{s.name} <span style={{ color: 'var(--text-tertiary)', fontSize: 12 }}>{s.code}</span></span>
                      <button onClick={() => onStockSymbolsChange(stockSymbols.filter((x) => x.code !== s.code))} style={{
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {POPULAR_STOCKS.filter((p) => !stockSymbols.some((s) => s.code === p.code)).map((s) => (
                    <button key={s.code} onClick={() => onStockSymbolsChange([...stockSymbols, s])} style={{
                      padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border-control)',
              ...
              <div>
                <div style={labelStyle}>自动刷新间隔</div>
                <select value={stockRefresh} onChange={(e) => onStockRefreshChange(Number(e.target.value))} style={selectStyle}>
                  {[10, 30, 60].map((n) => <option key={n} value={n}>{n} 秒</option>)}
                </select>
              </div>
              <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
        ...
        <div style={{
          ...
          padding: '12px 20px', borderTop: '1px solid var(--border-divider)',
        }}>
          <button onClick={onClose} style={{
            padding: '6px 20px', borderRadius: 6, border: '1px solid var(--border-control)',
            background: '#fff', fontSize: 13, cursor: 'pointer', color: 'var(--text-primary)',
          }}>
            取消
          </button>
...
function FloatingWidgetPreview({
  ...
  const config: Record<string, unknown> = currentStyle ? { style: currentStyle.id } : {}
...
function ClockPreview({ styleId }: { styleId?: string }) {
  ...
  return (
    <div style={{ ...base, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 300, letterSpacing: 5, opacity: 0.7, marginTop: 4 }}>{day} · {date}</span>
    </div>
  )
}
...
function AudioPreview({ styleId }: { styleId?: string }) {
  ...
  if (styleId === 'wave') {
    ...
    return (
      <svg width="280" height="60" viewBox="0 0 280 60" style={{ overflow: 'visible' }}>
        <polyline points={points} fill="none" stroke="#1a1a1a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.7" />
      </svg>
    )
  }
  if (styleId === 'spectrum') {
    ...
    const d = `M ${pts[0].x} ${pts[0].y} ` + pts.slice(1).map(p => `L ${p.x} ${p.y}`).join(' ') + ` L ${pts[pts.length - 1].x} 60 L 0 60 Z`
    return (
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 60 }}>
      {bars.map((h, i) => (
        <div key={i} style={{
          width: 6, height: h * 60, borderRadius: 3,
          background: 'linear-gradient(to top, #1a1a1a, #555)', opacity: 0.8,
        }} />
      ))}
    </div>
  )
...
function WeatherPreview({ styleId }: { styleId?: string }) {
  ...
  if (styleId === 'realism') {
    return (
      <div style={row}>
        <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #f59e0b, #ef4444)', opacity: 0.8, flexShrink: 0 }} />
  ...
  if (styleId === 'glass') {
    return (
      <div style={{
        ...row, padding: '10px 16px', borderRadius: 14,
  ...
  if (styleId === 'minimal') {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...circleOuter,
          background: '#f5f5f5', border: '2px solid rgba(0,0,0,0.08)',
...
function TextPreview() {
  return (
    <div style={{
      ...
      justifyContent: 'center', padding: 16, textAlign: 'center', width: '100%',
    }}>
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4, fontWeight: 600 }}>
        <span>CPU Util</span><span>34%</span>
      </div>
      <div style={{ width: '100%', height: 8, borderRadius: 4, background: 'var(--border-control)', marginBottom: 16, overflow: 'hidden' }}>
        <div style={{ width: '34%', height: '100%', background: 'var(--accent)' }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4, fontWeight: 600 }}>
        <span>Memory</span><span>12.4 GB / 32.0 GB</span>
      </div>
      <div style={{ width: '100%', height: 8, borderRadius: 4, background: 'var(--border-control)', overflow: 'hidden' }}>
...
function ToolIcon({ label }: { label: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer' }}>
      <div style={{
        width: 44, height: 44, borderRadius: '50%', background: 'var(--bg-solid)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        marginBottom: 6, border: '1px solid var(--border-control)',
      }}>
        <Wrench size={18} />
      </div>
